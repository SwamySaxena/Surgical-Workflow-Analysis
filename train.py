...
parser.add_argument('--best_ep', default=9, type=int)
...
num_layers = 12 # layers of prediction tcn e, org = 12
num_f_maps = 64
dim = 2048
...
num_layers_PG = 11 #args.num_layers_PG
num_layers_R = 10 #args.num_layers_R
num_R = args.num_R
...
def base_train(model, train_loader, validation_loader, save_dir = 'models/base_tcn', debug = False):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for (video, labels, mask, video_name ) in (train_loader):
                ...
                # Repeat labels to match the total number of elements in the reshaped_outputs tensor  epeated_labels = labels.repeat(3 * 7)
                # outputs = outputs[4:]
                # print(outputs.shape)
                # print(labels.shape)
                
                #output_tensor = outputs.transpose(2, 1).contiguous().view(-1, num_classes)
                #print(output_tensor.shape)
                
                loss += loss_layer(outputs.transpose(2, 1).contiguous().view(-1, num_classes), labels.view(-1)) # cross_entropy loss
                #loss += loss_layer(output_tensor, labels.view(-1)) # cross_entropy loss
                loss += 1.5*torch.mean(torch.clamp(mse_layer(F.log_softmax(outputs[:, :, 1:], dim=1), F.log_softmax(outputs.detach()[:, :, :-1], dim=1)), min=0, max=16)) # smooth loss
...
if args.model=="Base":
    ...
    #base_model = ms_tcn.End_to_End_Model(num_stages, num_layers, num_f_maps, dim, num_classes)
...
if args.action == 'base_train':
    ...
    video_train_feature_folder = 'surgical_code/features/{}/{}/train_dataset/video_feature@2020@{}'
    ...
    video_test_folder = 'F:/cholec80/test_dataset'
    video_test_feature_folder = 'surgical_code/features/{}/{}/test_dataset/video_feature@2020@{}'
...
if args.action == 'base_predict':
    ...
    video_folder = 'F:/cholec80/test_dataset'
    video_feature_folder = 'surgical_code/features/{}/{}/test_dataset/video_feature@2020@{}'
    video_testdataset = VideoDataset(args.dataset, root_path, root_path+annotation_path.format(args.dataset),\
