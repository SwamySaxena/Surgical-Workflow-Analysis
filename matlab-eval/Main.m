...
phaseGroundTruths = {};
gt_root_folder = 'D:/SEM-VII0/Project/datasets/surgical/workflow/cholec80/phase_annotations/'; %annotation_folder
for k = 41:80
    num = num2str(k);
    to_add = ['video' num '-phase'];
...
for i = 1:length(phaseGroundTruths)
    predroot = 'D:/SEM-VII0/Project/surgical_code/results/cholec80/resnet50_noIN_epoch9_s1_e41/prediction_5/'; % your prediction folder

    %predroot = '../../Results/multi/phase';
    %predroot = 'D:/SEM-VII0/Project/surgical_code/results/cholec80/resnet50_noIN_epoch9_s1_e41/prediction_5';
    phaseGroundTruth = phaseGroundTruths{i};
    num = cell2mat(regexp(phaseGroundTruth,'\d', 'match'))
    predFile = [predroot phaseGroundTruth(end-11:end-10)];
    predFile = [predFile '_pred.txt']
